    ...
    def doSimplex(self):
        ...
        for h in range(len(self.raw_indexes)-1):
            if(getIntegerIndex(self.raw_indexes[h])<=len(self.c)):
               ...
               print(self.raw_indexes[h],"=",round(self.Table[h][0],7),end=" ")
    ...
    def printAnswer(self):
        ...
        for h in range(1,len(self.column_indexes),1):
          if(getIntegerIndex(self.column_indexes[h])<=len(self.c)):
               print(self.column_indexes[h],"=0",end=" ")
        print("")
    def Gomori(self,notInteger):
        raw_ind=0
        for h in range(len(self.raw_indexes)-1):
            if notInteger==self.raw_indexes[h]:
                  raw_ind=h
        newLine=[]
        for i in range(len(self.column_indexes)):
            newLine.append(-(self.Table[raw_ind][i]-floor(self.Table[raw_ind][i])))
        self.Table.insert(len(self.raw_indexes)-1,newLine)
       
        self.raw_indexes.insert(len(self.raw_indexes)-1,"x"+str(len(self.raw_indexes)+len(self.column_indexes)-1))
